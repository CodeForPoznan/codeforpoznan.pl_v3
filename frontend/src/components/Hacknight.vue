            ...
            <v-col class="d-flex" cols="12" sm="6">
              ...
              <template v-if="noData" v-slot:no-data>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-title>
                      Select or create hacknight first. No results matching "<strong>{{ search }}</strong>". Press <kbd>enter</kbd> to create a new one
                    </v-list-item-title>
                  </v-list-item-content>
                </v-list-item>
              </template>
        ...
        >
          ...
          >
            <v-avatar color="blue" size="48" style="margin-right:1em;">
              <span class="white--text headline">{{participants.length}}</span>
            </v-avatar>
...
export default {
    ...
    methods: {
      onCreateHacknight() {
        this.$store.dispatch('hacknight/createHacknight')
        .then(res => {
          this.participants = []
      ...
      onAddParticipants() {
        ...
        this.$store.dispatch('hacknight/addParticipants',  params)
        .then(res => {
