...
def test_get_delete_put_non_existing_team(
    auth_client, add_teams, method
):
...
def test_add_members_to_team_unauthorized(
    add_teams, add_participants, client
):
...
def test_duplicate_member_in_team(
    auth_client, add_teams, add_participants, _db
):
    ...
    rv = auth_client.post(
        f"/api/teams/{team.id}/members/", data=json.dumps(payload),
    )
    response = rv.get_json()
...
def test_remove_all_members_from_team(
    auth_client, add_members_to_team
):
